...
rethinking_samplesize_full <- ulam(
    alist(
        Z_obs ~dnorm ( Z_true , Z_sd),
        vector[N_spp]:Z_true ~ dnorm( mu , sigma_sq ),
        mu <- a+y*SampleSize + c[C]+m[M]+l*L,
        a~dnorm(0,1),
        y~dnorm(0,1),
        l~dnorm(0,1),
        sigma_sq~dexp(1),
      ## adaptive priors
        s[Study] ~dnorm(0, sigma_study),
