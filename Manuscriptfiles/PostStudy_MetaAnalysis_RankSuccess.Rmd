...
The sample bias, the overrepresentation of extreme effect sizes, in our data likely results from all three influences of publication bias, study system bias, and study time bias. For publication bias, our univariate analyses indicate a number of study systems in which effect sizes are different. In some cases, these systems are overrepresented: for example, while less than 5% of all mammalian species are cooperative breeders, 12% of all effect sizes in our sample are from cooperative breeders. For study system bias, we find that the noise in the estimate is much reduced in studies that were conducted across longer periods.

To identify the potential interplay between the three biases, we build combined models based on the predicted causal relationships. 
