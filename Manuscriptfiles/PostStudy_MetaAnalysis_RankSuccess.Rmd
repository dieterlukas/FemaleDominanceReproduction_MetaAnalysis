...
As a further indication of 'publication bias', we find that studies with small sample sizes and small effect sizes (those that presumably did not reach statistical significance) are missing in our dataset such that the average effect sizes at smaller sample sizes are more extreme than those at larger sample sizes (metafor estimate 95% confidence interval lower -0.03 to upper -0.02, rethinking estimate 89% compatibility estimate of posterior sample lower -0.09 to upper -0.04) (Figure 2). The estimate of the intercept and slope of this model though show that across the range of sample sizes the estimate of the overall effect size does not go below zero (see line in Figure 2), because effect size estimates with large sample sizes tend to be positive. This suggests that if even if those small or negative effect sizes with small sample sizes that appear to be missing from our dataset due to publication bias, the data would still support that overall there is a positive relationship between dominance rank and female reproductive success across studies.
...
(iii) Studies which measured dominance rank categorically by classifying individuals as either dominants or subordinates report higher effect sizes (metafor estimate +0.29 - +0.35, rethinking estimate +0.31 - +0.41; n=251 effect sizes) than studies assigning individuals continuous ranks (metafor estimate 0.16-0.22, rethinking estimate +0.17 - +0.28; n=193 effect sizes). In essentially all studies of cooperative breeders (31 of 32 effect sizes), comparisons were between the single dominant female and a class of the remaining subordinate females, which may contribute to higher effect sizes for studies using categorical measures of rank (see section R5.2.1).
...
Group size and sex ratio have independent influences on effect sizes (group size estimate metafor -0.01 - 0.00, rethinking -0.07 - -0.02; sex ratio estimate metafor +0.53 - +1.18, rethinking +0.06 - +0.11; n=346 effect sizes), while there is no support for an interaction between the two (interaction estimate metafor -0.02 - +0.02, rethinking -0.03 - 0.04).
...
Population density but not the diet category are associated with variation in the effect of dominance rank on reproductive success (population density estimate metafor 0.00 - +0.01, rethinking +0.05 - +0.11; diet category estimate metafor -0.31 - +0.21, rethinking -0.40 - +0.69; n=346 effect sizes), while there is no support for an interaction between the two (interaction estimate metafor -0.02 - +0.02, rethinking -0.03 - 0.04).
...
Population density but not environmental harshness are associated with variation in the effect of dominance rank on reproductive success (population density estimate metafor 0.00 - +0.01, rethinking +0.04 - +0.11; environmental harshness estimate metafor -0.10 - +0.07, rethinking -0.08 - +0.01; n=214 effect sizes), and there is no support for an interaction between the two (interaction estimate metafor -0.001 - +0.001, rethinking -0.09 - +0.01).
